...
void* check_increasing(void* arg) {
    ...
    while(1) {
        ...
        //////////////////
        pthread_rwlock_rdlock(&nd->sync);
        while (nd->next != NULL) {
            node_t* nd2 = nd->next;
            pthread_rwlock_rdlock(&nd2->sync);
            if (strlen(nd->value) < strlen(nd2->value)) {
                count++;
            }
            pthread_rwlock_unlock(&nd->sync);
            nd = nd2;
        }
        pthread_rwlock_unlock(&nd->sync);
...
void* swap(void* arg) {
    ...
    while(1) {    
        swap1 = 0;
        node_t* nd = list->first;
        pthread_rwlock_wrlock(&nd->sync);
        node_t* nd2 = nd->next;
        pthread_rwlock_wrlock(&nd2->sync);
        
        while (nd2->next != NULL) {
            
            node_t* nd3 = nd2->next;
            pthread_rwlock_wrlock(&nd3->sync);
            if (rand() % 2 == 1) {       
                nd2->next = nd3->next;
                nd3->next = nd2;
                nd->next = nd3;
        ...
        printf("swap iter = %d\n", swap1);
        pthread_rwlock_unlock(&nd->next->sync);
        pthread_rwlock_unlock(&nd->sync);     
   }
}
